  ...
  function chart(selection) {
    selection.each(function(data) {
      ...
      var pieLabels = wrap.select('.nv-pieLabels').selectAll('.nv-label')
          .data(pie);
      var pieValueLables = wrap.select('.nv-pieLabels').selectAll('.nv-valueLabel')
          .data(pie);
      ...
      var ae = slices.enter().append('g')
        ...
        if (showLabels) {
          ...
          pieLabels.enter().append("g").classed("nv-label",true)
            ...
            pieValueLables.enter().append("g").classed("nv-valueLabel",true)
                .each(function(d,i) {
                    var group = d3.select(this);
                    ...
                    group
                        .attr('transform', function(d) {
                            if (labelSunbeamLayout) {
                                ...
                                var px = labelsArc.centroid(d)[0];
                                var py = labelsArc.centroid(d)[1]+20;
          ...
          pieLabels.select(".nv-label text")
            ...
            pieValueLables.select(".nv-valueLabel text")
                .style('text-anchor', labelSunbeamLayout ? ((d.startAngle + d.endAngle) / 2 < Math.PI ? 'start' : 'end') : 'middle') //center the text on it's origin or begin/end if orthogonal aligned
                .text(function(d, i) {
                    var percent = (d.endAngle - d.startAngle) / (2 * Math.PI);
                    var labelTypes = {
                    ...
                    return (d.value && percent > labelThreshold) ? labelTypes["percent"] : '';
                }
            );
        }
